...
const NAV_ITEMS = [
  { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={20} />, path: '/volunteer', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
  { id: 'students', label: 'My Students', icon: <Users size={20} />, path: '/volunteer/students', color: 'text-cyan-500', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20' },
  { id: 'submissions', label: 'Submissions', icon: <FileCheck size={20} />, path: '/volunteer/submissions', color: 'text-indigo-500', bg: 'bg-indigo-500/10', border: 'border-indigo-500/20' },
  { id: 'settings', label: 'Settings', icon: <Settings size={20} />, path: '/volunteer/settings', color: 'text-slate-400', bg: 'bg-slate-400/10', border: 'border-slate-400/20' },
];
    ...
    <>
      ...
      >
        <div className="h-20 flex items-center px-4 mb-4 mt-2">
          <div className="flex items-center justify-center min-w-[44px] h-[44px] rounded-xl bg-accent/10 text-accent ring-1 ring-accent/20 shrink-0">
            <ShieldCheck size={24} />
          </div>
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                <span className="font-bold tracking-tight text-foreground text-lg leading-tight">Volunteer</span>
            ...
            <AnimatePresence>
              {isExpanded && (
                <motion.div
                  ...
                  className="ml-3 flex flex-col whitespace-nowrap overflow-hidden shrink-0"
                >
                  <span className="text-sm font-semibold text-foreground truncate max-w-[150px]">{user?.displayName || 'Volunteer'}</span>
                  <span className="text-xs text-primary font-medium">Reviewer</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
          ...
          return (
            ...
            >
              <div className="relative">
                {active && (
                  <motion.div 
                    layoutId="active-mobile-volunteer"
