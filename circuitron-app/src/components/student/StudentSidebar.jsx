...
const NAV_ITEMS = [
  { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={20} />, path: '/dashboard', color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
  { id: 'tasks', label: 'My Tasks', icon: <CheckSquare size={20} />, path: '/dashboard/tasks', color: 'text-emerald-500', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
  { id: 'leaderboard', label: 'Leaderboard', icon: <Trophy size={20} />, path: '/dashboard/leaderboard', color: 'text-amber-500', bg: 'bg-amber-500/10', border: 'border-amber-500/20' },
  { id: 'settings', label: 'Settings', icon: <Settings size={20} />, path: '/dashboard/settings', color: 'text-slate-400', bg: 'bg-slate-400/10', border: 'border-slate-400/20' },
];
        ...
        <div className="h-20 flex items-center px-4 mb-4 mt-2">
          ...
          <AnimatePresence>
            {isExpanded && (
              ...
              >
                <span className="font-bold tracking-tight text-foreground text-lg leading-tight">Student</span>
            ...
            <AnimatePresence>
              {isExpanded && (
                <motion.div
                  ...
                  className="ml-3 flex flex-col whitespace-nowrap overflow-hidden shrink-0"
                >
                  <span className="text-sm font-semibold text-foreground truncate max-w-[150px]">{user?.displayName || 'Student'}</span>
                  <span className="text-xs text-primary font-medium">{displayLevel} Level</span>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
